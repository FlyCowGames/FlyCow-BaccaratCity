...
## Session 63 (2026-03-22) -- Firework Sound Effects
- **Lesson:** Adding sound effects to an existing visual feature (fireworks) is higher-impact than adding the 22nd billboard entity. The fireworks went from silent to multi-sensory — same lesson as Session 58 (baccarat sounds). Deepening existing features > adding breadth.
- **Lesson:** Procedural firework sounds require two distinct components: (1) a rising whistle/whoosh for the launch (sine sweep + highpass noise), and (2) an explosive pop for the burst (lowpass noise burst + delayed crackle). The crackle tail (sparse random pops at 2% density through bandpass filter) adds realism without continuous noise.
- **Lesson:** Randomizing sound parameters per firework (frequency range ±random, gain ±random, crackle delay ±random) prevents repetitive audio. Even small variations (200+random*100 Hz base frequency) make each firework sound distinct.
- **Lesson:** The `playFireworkSound` function follows the exact same pattern as `playBaccSound` from Session 58 — noise buffers for percussive sounds, oscillators for tonal sounds, exponentialRampToValueAtTime for decay. The Web Audio API procedural sound pattern is now proven across 3 features (ambient, baccarat, fireworks).
- **Observation:** This is the 3rd audio feature (ambient S9, baccarat S58, fireworks S63). The project now has a solid multi-layer audio system: ambient background + interactive sounds + environmental sounds. Future candidates for sound: helicopter rotor hum, LRT bell, temple bells.
- **Observation:** Session 60 audit flagged daytime experience as weak. This session's improvement (firework sounds) only benefits nighttime visitors (20:00-01:00). Future sessions should prioritize daytime-visible/audible improvements to address the audit finding.

## Session 64 (2026-03-22) -- WASD Walking & Mobile Joystick
- **Lesson:** Enhancing an existing meta-feature (street mode → walking controls) is higher-impact than adding a 23rd billboard entity. Street mode was functional but passive — you could only look around and scroll-zoom. WASD walking makes it actively explorable, fulfilling the Prime Directive's "explorable" goal.
- **Lesson:** CesiumJS `camera.heading` gives the direction the camera faces. For walking, movement is along heading (forward) and perpendicular (strafe). Converting heading-relative movement to lon/lat offsets uses `moveEast = sin(heading)*fwd + cos(heading)*right` and `moveNorth = cos(heading)*fwd - sin(heading)*right`, then dividing by Earth's radius (6378137m) for radian offsets.
- **Lesson:** `viewer.clock.onTick` is the correct hook for per-frame walking movement — same event used for orbit camera (Session flythrough). Delta time capping (`if (dt > 0.1) dt = 0.1`) prevents large jumps when the tab loses focus.
- **Lesson:** When adding WASD walking, existing keyboard shortcuts that use the same keys (S=share, D=day preview, W/A could be future shortcuts) MUST be bypassed in street mode. Added an early return check in the shortcut handler for WASD/arrow keys when `_streetMode` is true.
- **Lesson:** Mobile virtual joystick using raw touch events is simple: track one active touch ID, compute displacement from center, normalize to -1..1 vector, cap at max radius. The joystick feeds into the same movement system as WASD keys — no separate movement code needed.
- **Lesson:** `'ontouchstart' in window` is a quick way to detect touch-capable devices and only show the joystick on mobile. In headless browser testing this returns false, so the joystick won't appear in automated screenshots — must force-show via JS for testing.
- **Observation:** The project now has 8 UX/meta-features (time slider S31, auto-tour S34, URL sharing S48, minimap S49, search S51, shortcuts S53, street mode S62, WASD walking S64). The interactivity layer is very mature. Future sessions should focus on visual/atmospheric improvements or deepening existing features.
- **Observation:** Walking speed of 8 m/s (~29 km/h) is faster than real walking (~1.4 m/s) but feels right for exploration at this scale. Too slow and users get frustrated; too fast and they overshoot.
- **Suggestion for future sessions:** Street mode still needs: (1) terrain-following (camera height clamps to ground surface, not fixed 5m), (2) sprint key (Shift to double speed), (3) mouse-look on right-click for desktop. These are enhancements for future sessions. Also: daytime experience is still weak per S60 audit — prioritize a daytime-visible feature next.

## Session 65 (2026-03-22) -- Typhoon Signal Weather Effects
- **Lesson:** Enhancing the existing weather system (already rated GOOD in S60 audit) is high-impact because it deepens a proven feature rather than adding another billboard entity. The weather system had wind data available but unused — the typhoon signals give that data a visible purpose.
- **Lesson:** CSS `transform: skewX()` on the rain overlay is a simple, performant way to simulate wind-driven rain. No need to modify individual rain drop elements — the transform applies to the container.
- **Lesson:** Canvas-based wind streaks (horizontal speed lines) use `requestAnimationFrame` for smooth animation. Unlike billboard entities, canvas overlays can render hundreds of moving elements without CesiumJS entity overhead. The same pattern as fireworks (S13) works for any screen-space particle effect.
- **Lesson:** The Typhoon Demo button pattern (toggle simulation with a button) is useful for any weather/time-dependent feature that visitors may not see during their visit. Like the time slider (S31) which makes night features accessible during day, the typhoon demo makes extreme weather features accessible during calm weather.
- **Lesson:** The typhoon signal system uses gradual effect scaling (fog density, light dimming, streak count all proportional to signal level) rather than binary on/off. This creates a convincing progression from T1 (subtle) to T10 (extreme) that feels realistic.
- **Lesson:** Function declarations are hoisted in JavaScript, so calling `updateTyphoonSignal()` from `updateWeatherUI()` before the function definition works fine. Arrow functions or `var = function()` would NOT hoist — always use `function name() {}` for functions that need to be called before their definition in a single-file app.
- **Observation:** This is the first roadmap item from "Medium-Term" to be completed since street-level mode (S62). Checking off roadmap items feels productive and gives the project a sense of progress toward the Prime Directive.
- **Observation:** The wind streaks canvas at z-index 48 sits below the rain overlay (50) and fireworks canvas (55). This layering is intentional — wind streaks are background atmosphere, rain is foreground, fireworks are above both. Maintaining z-index discipline prevents visual conflicts.
- **Suggestion for future sessions:** The typhoon system could be enhanced with: (1) wind sound effects (howling wind via Web Audio), (2) swaying landmark billboards, (3) rough seas (wave entity amplitude increase), (4) event integration (typhoon season July-November). Also: the daytime experience is still the weakest area — consider Portuguese colonial district detail or Coloane village next.

## Session 66 (2026-03-22) -- Night Sky Stars & Shooting Stars
- **Lesson:** Enabling `skyBox.show = true` in CesiumJS activates the built-in star field cubemap. The default CesiumJS Viewer creates a SkyBox with star textures from the CDN automatically — no need to provide explicit texture URLs. With `skyAtmosphere.show = true`, stars are naturally hidden during daytime (atmospheric scattering covers them) and visible at night.
- **Lesson:** The canvas overlay pattern (requestAnimationFrame + canvas at a specific z-index) is now proven across 4 features: fireworks (S13, z55), wind streaks (S65, z48), shooting stars (S66, z45). Each uses the same structure: an IIFE with local state, spawn/update/render loop, and time-gated activation. This is the go-to pattern for any screen-space particle effect.
- **Lesson:** Shooting star trails use `ctx.createLinearGradient()` along the trail direction for a fade effect from bright head to transparent tail. This is more realistic than a single-color line.
- **Lesson:** Spawn rate of ~0.012 per frame (~0.7/sec at 60fps) averages to about 1 meteor every 1-2 seconds. This feels right — frequent enough to notice, sparse enough to feel special. Higher rates would look like a meteor shower, lower rates might be missed.
- **Observation:** This session was simple and focused — 2 changes (skyBox enable + shooting star canvas) totaling ~80 lines. Not every session needs a complex 200+ line feature. Small atmospheric improvements can be high-impact with minimal code.
- **Observation:** Canvas overlay z-index stack is now: shooting stars (45) < wind streaks (48) < rain (50) < fireworks (55). Each layer has a clear purpose and they don't interfere with each other.
- **Suggestion for future sessions:** The night sky could be further enhanced with: (1) northern lights / aurora effect during special events, (2) moon phase tracking (CesiumJS moon is already enabled but could be enhanced), (3) constellation labels when zoomed into street mode looking up. Also: daytime experience still needs attention — Portuguese colonial district, Coloane village, or daytime atmospheric effects (god rays, cloud shadows).

## Session 67 (2026-03-22) -- Feature Tips Discovery System
- **Lesson:** The site has 30+ features but no onboarding. A rotating tip system is a meta-feature that increases discoverability of ALL existing features without adding visual complexity. This is high leverage — one feature makes every other feature more discoverable.
- **Lesson:** `'ontouchstart' in window` is a reliable way to serve device-appropriate tips (tap vs keyboard instructions). The same tip array with conditional text works better than maintaining separate tip sets.
- **Lesson:** Timing matters for tips: 25-second delay (after flythrough) ensures users aren't overwhelmed during the cinematic intro. 10-second rotation gives enough reading time. 5-minute auto-hide respects users who've learned the basics.
- **Lesson:** The opacity transition technique (set `el.style.opacity = '0'`, wait 600ms, update content, remove inline opacity to let CSS class take over) creates clean fade-out/fade-in transitions without complex animation logic.
- **Observation:** The project now has 9 UX/meta-features (time slider, auto-tour, URL sharing, minimap, search, shortcuts, street mode, WASD walking, feature tips). The interactivity/UX layer is very mature. Future sessions should strongly prioritize visual/atmospheric improvements — specifically daytime features, since most visual content is night-only.
- **Observation:** Session 70 will be the next FULL QUALITY AUDIT (70 % 10 == 0). Must not skip.
- **Suggestion for future sessions:** The daytime experience is still the weakest area. Candidates: (1) Portuguese colonial district detail at Senado Square, (2) Coloane village and Hac Sa beach, (3) daytime atmospheric effects (god rays, cloud shadows, golden hour color grading), (4) street market stalls/food stands, (5) animated construction vehicles at crane sites. Prioritize something visible and daytime-specific.

## Session 68 (2026-03-22) -- Golden Hour & Cinematic Color Grading
- **Lesson:** A simple CSS overlay with dynamic `rgba()` background is the lightest-weight approach for scene-wide color grading. No canvas, no WebGL post-processing, no performance impact. The `z-index: 42` placement below all canvas effects (shooting stars 45, wind streaks 48, rain 50, fireworks 55) ensures proper layering.
- **Lesson:** Color grading with 8 distinct time phases creates smooth cinematic transitions through the full day cycle. The key is interpolating RGB values between phases with gradual alpha transitions — hard cuts between phases would look jarring. The 4-second CSS opacity transition handles cross-phase smoothing.
- **Lesson:** `mix-blend-mode: multiply` was initially considered but removed — it darkens everything, making bright skies look muddy. A simple semi-transparent rgba overlay is better for this use case because it adds color without crushing highlights.
- **Lesson:** 5-second update interval via `setInterval` is sufficient for color grading — the transitions are gradual (hours-scale changes) so per-frame updates would be wasteful. This matches the neon glow update (10s) and weather update (15min) pattern of "update frequency matches rate of change."
- **Observation:** This directly addresses the most repeated feedback across sessions 60-67: "daytime experience is weak." The golden hour effect makes time-of-day transitions visually striking — the warm amber glow during sunset hours is immediately noticeable and cinematic.
