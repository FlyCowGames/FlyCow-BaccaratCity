...
**Tags:** performance, css, animation | Applied: 8/8 successful
...
**Tags:** css, animation, visual-quality | Applied: 10/10 successful
...
**Tags:** audio, performance | Applied: 5/5 successful

### Enhancing existing features is high-impact with low risk [w=0.7]
...
**Tags:** architecture, composability, feature-enhancement | Applied: 20/20 successful

### Cultural balance through complementary heritage elements [w=0.7]
...
**Tags:** culture, heritage, visual-variety, authenticity | Applied: 4/4 successful

### Time-of-day gated features fill temporal gaps and add realism [w=0.7]
...
**Tags:** animation, time-of-day, cultural-authenticity, billboard | Applied: 4/4 successful

### New landmark integrates with all existing night-effect systems automatically [w=0.7]
...
**Tags:** landmarks, data-driven, architecture, night-effects | Applied: 4/4 successful

### Layered weather effects compound for immersion [w=0.65]
...
**Tags:** atmosphere, weather, css, layering | Applied: 4/4 successful
...
**Tags:** css, animation, cinematic, typography | Applied: 1/1 successful
...
**Tags:** canvas, rendering, neon, performance | Applied: 3/3 successful

### Time-lapse mode showcases time-gated features visitors would otherwise miss [w=0.6]
...
**Tags:** interactivity, time-of-day, ux, showcase, requestAnimationFrame | Applied: 1/1 successful

### Static billboard entities for signage need no animation interval [w=0.6]
...
**Tags:** performance, billboard, canvas, simplicity | Applied: 2/2 successful

### Adding missing landmarks to LANDMARKS array fixes walk tour gaps [w=0.6]
...
**Tags:** landmarks, walk-tour, data-driven, composability | Applied: 2/2 successful

### Landmark-specific action buttons extend info cards into interactive experiences [w=0.6]
...
**Tags:** interactivity, ux, landmarks, viewpoints, orbit | Applied: 2/2 successful

### Non-casino cultural landmarks diversify the landmark collection [w=0.6]
...
**Tags:** landmarks, cultural-diversity, data-driven, simplicity | Applied: 5/5 successful

### Audit sessions catch mobile UI overlap regressions that accumulate over time [w=0.6]
...
**Tags:** mobile, css, audit, layout, overlap | Applied: 1/1 successful

### localStorage persistence for game state enhances engagement with minimal code [w=0.6]
...
**Tags:** localStorage, interactivity, persistence, baccarat, engagement | Applied: 2/2 successful

### Meta-UI that surfaces existing features increases perceived value [w=0.6]
...
**Tags:** interactivity, ux, discoverability, meta-ui, time-of-day | Applied: 1/1 successful
...
**Tags:** cesiumjs, animation, architecture | Applied: 1/1 successful
...
**Tags:** ux, ui, tour | Applied: 1/1 successful
...
**Tags:** canvas, animation, atmosphere, performance | Applied: 1/1 successful

### Position-aware audio zones using proximity falloff with altitude gate [w=0.5]
...
**Tags:** audio, spatial, cesiumjs, immersion | Applied: 1/1 successful

### Official symbols and emblems add cultural depth [w=0.5]
...
**Tags:** culture, heritage, symbolism, authenticity | Applied: 1/1 successful

### Heritage district ground-level transport adds cultural authenticity [w=0.5]
...
**Tags:** animation, heritage, transport, cultural-authenticity | Applied: 1/1 successful

### Distributed small light points create coastline-tracing glow effects [w=0.5]
...
**Tags:** atmosphere, lighting, night, waterfront, billboard | Applied: 1/1 successful

### CSS cloud overlays are effective weather-reactive sky atmosphere [w=0.5]
...
**Tags:** css, atmosphere, weather, performance, animation | Applied: 1/1 successful

### Larger canvas entities with detail reward night scenes [w=0.5]
...
**Tags:** canvas, rendering, night, scale, billboard | Applied: 1/1 successful

### Canvas forked lightning with recursive branching creates realistic bolts [w=0.5]
...
**Tags:** canvas, atmosphere, weather, audio, animation | Applied: 1/1 successful

### CSS Grid with explicit row/column placement for tabular game data [w=0.5]
...
**Tags:** css-grid, interactivity, baccarat, responsive | Applied: 1/1 successful

### CSS particle celebrations reward user actions and reinforce engagement [w=0.5]
...
**Tags:** css, animation, interactivity, ux, celebration | Applied: 1/1 successful

### Regulatory/safety details add skyline authenticity [w=0.5]
...
**Tags:** atmosphere, authenticity, night, billboard, regulatory | Applied: 1/1 successful

### CSS crepuscular rays enhance golden hour without entity overhead [w=0.5]
...
**Tags:** css, atmosphere, golden-hour, cinematic, performance | Applied: 1/1 successful

### Multi-layer CSS fog with staggered drift creates convincing mist [w=0.5]
...
**Tags:** css, atmosphere, fog, animation, layering | Applied: 1/1 successful

### CSS radial-gradient creates natural rainbow bands [w=0.5]
...
**Tags:** css, atmosphere, weather, radial-gradient | Applied: 1/1 successful

### Complementary features in the same area compound visual richness [w=0.5]
...
**Tags:** landmarks, visual-richness, complementary, night-market | Applied: 1/1 successful

### Casino-branded shuttle buses with route-specific colors add transport variety [w=0.5]
...
**Tags:** animation, transport, canvas, cultural-authenticity, billboard | Applied: 1/1 successful

### Speed variation in transport entities adds visual dynamism [w=0.5]
...
**Tags:** animation, transport, harbour, speed-contrast | Applied: 1/1 successful

### Gamification via discovery tracking encourages exploration of existing content [w=0.5]
...
**Tags:** gamification, localStorage, engagement, ux, exploration | Applied: 1/1 successful

### Conditional compound weather effects add cinematic depth [w=0.5]
...
**Tags:** atmosphere, weather, css, night, compound-conditions, cinematic | Applied: 1/1 successful

### CSS radial gradient drops with streaking animation simulate camera lens effect [w=0.5]
...
**Tags:** css, atmosphere, weather, animation, cinematic, camera-effect | Applied: 1/1 successful
...
- **How to add authentic skyline detail to tall buildings at night?** → Red radial gradient billboard entities at building-top altitudes with slow blink via CallbackProperty, night-only glow gating — Aviation obstacle lights are mandatory on tall buildings worldwide — every Macau skyscraper has them. Follows exact bridge lights pattern (CallbackProperty scale, _currentGlowIntensity gating). Simple 3s blink cycle (1.5s on/1.5s dim) with staggered phases per building prevents synchronization. Small 12px canvas icon is visible from overview but unobtrusive.
- **How to add early morning atmosphere beyond sun rays?** → Multi-layer CSS fog/mist overlay with radial gradients, blur filters, and staggered drift animations at bottom of viewport — CSS approach avoids more CesiumJS entities. Multiple layers with different heights, speeds, and opacities create depth that a single fog div cannot. Humidity/fog weather reactivity makes it feel connected to real conditions. Complements (not competes with) the existing sun rays at dawn — rays are geometric beams above, mist is soft blanket below.
- **How to add a rainbow weather effect to complement the rain system?** → CSS radial-gradient div with 7 spectral bands, condition-gated to daytime + light rain + partial sun, with 10-min post-rain memory — CSS approach (no canvas/entity) follows proven weather overlay pattern. radial-gradient is simpler and more natural than conic-gradient for concentric rainbow bands. Post-rain memory (10 min window) is key — real rainbows often appear as rain stops, not during heavy rain. Cloud cover threshold (85%) ensures sun visibility is plausible.
- **How to add night market ambiance to food areas?** → Canvas-drawn string light billboard entities with catenary wire and warm golden twinkle bulbs at 7-9m altitude — String lights are universally recognizable night market signifiers. Canvas + setInterval pattern is proven reliable. Placing at existing food/heritage locations (Rua do Cunha, Senado Square, etc.) compounds with food stalls and red lanterns for layered richness. Always-active with night boost means visible in all conditions.
- **How to let visitors experience all time-gated features without visiting at different hours?** → requestAnimationFrame time-lapse animating _timeOverride 0-24 over 48 seconds, small play button next to time slider — Reuses existing time override system (slider, updateCesiumClock, updateNeonGlow) — zero new infrastructure. Placing the button in the clock block (next to LIVE) is contextually obvious. 48s duration (2s/hour) is long enough to appreciate changes but short enough to watch in full. Returns to live time on completion for clean UX.
- **How to deepen Portuguese colonial heritage beyond decorative tiles?** → Bilingual street name sign billboards at 8 famous intersections, canvas-drawn with tile border aesthetic, static (no animation) — Street name signs are one of the most photographed features of historic Macau. Blue-and-white tile style matches azulejo panels (S82). Static billboards (no setInterval) because signs don't animate. 6m altitude matches real sign height. Portuguese + Chinese text reinforces the bilingual identity. Complements azulejo tiles, food stalls, and red lanterns in the heritage district.
- **How to add iconic Macau ground transport beyond tour buses and triciclos?** → Casino free shuttle buses (發財車) on 3 routes between ferry terminals and casinos, always active with casino-branded colors — Casino shuttles are the most ubiquitous Macau transport — every tourist uses them. Always-active (24/7) fills gap since tour buses are midday-only. Per-route casino branding (gold/Lisboa, purple/Galaxy, blue/COD) adds visual variety. Follows proven CallbackProperty ping-pong pattern from ferries/triciclos.
- **How to add traditional Chinese architectural elements to complement Portuguese heritage features?** → Canvas-drawn paifang (牌坊) gateway arch billboards at 5 key Macau entrance locations, static (no animation) — Paifang are iconic Chinese architectural gateways found at important entrances throughout Macau. Complements Portuguese heritage (azulejo tiles, street signs) with Chinese counterpart. Static billboard pattern from S114 street signs — no setInterval needed for non-animated structures. Red/gold/green color palette provides warm contrast to blue-white Portuguese elements. 5 diverse locations (Border Gate, food street, temple, neighborhood, historic street) spread the feature geographically.
- **Which landmark to add to complete the Prime Directive casino list?** → MGM Macau (Peninsula) as a full LANDMARKS entry with info card, photos, and integration into all night-effect systems — MGM was the last casino explicitly named in the Prime Directive that was missing from LANDMARKS. The Peninsula location (not MGM Cotai) is the original and more iconic property with the wave-shaped golden facade and 10m lion. Adding it as a data-driven landmark (array entries only, no new code) leveraged the existing architecture for maximum impact with minimal risk.
- **How to let visitors experience the iconic Macau Tower panoramic view?** → Camera flight to 240m + slow onTick orbit with cinematic CSS overlay, triggered by landmark card button — The observation deck is Macau Tower's defining experience. Reusing the orbit pattern (separate _towerOrbitHandle like walk tour) keeps it independent from other sequences. Small orbit radius (0.0004°) keeps camera near the tower while rotating. Button in the existing landmark card is the natural discovery point. Blue accent color distinguishes from gold casino buttons. Future: could add similar viewpoints for other landmarks (e.g., Golden Reel ride at Studio City).
- **How to add speed variety to harbour water traffic?** → Water taxi speedboats with shorter cycle times (28-35s) on 2 harbour routes, same CallbackProperty ping-pong pattern as ferries — All existing water transport is slow (ferries 50-55s, junks 85-100s, dinner cruise 120s). Water taxis are authentic Macau harbour transport and their speed (nearly 2x ferries) creates visible contrast. Canvas wake spray trail differentiates them visually. Always-active (not time-gated) adds constant harbour dynamism. Bright blue minimap dot is distinct from cyan ferry dots.
- **How to make baccarat more engaging for returning visitors?** → Persist bankroll and game stats in localStorage, show session stats and welcome back indicator — Without persistence, every visit starts at HK$10,000 with no sense of history. localStorage is the simplest persistence mechanism — no server needed, works offline, instant. Tracking meta-stats (hands played, biggest win, peak bankroll) adds a sense of progression beyond just the current balance. The welcome back message is subtle (4s fade) — enough to feel personalized without being intrusive. Reset button provides a clean escape hatch.
- **How to make the time-gated feature system more discoverable?** → City Pulse expandable overlay with feature list, active/inactive status, click-to-fly, and next-up preview — The project has 11+ time-gated features but visitors only see what's active at the current Macau hour. A meta-UI that reads getMacauHour() against feature time windows surfaces the temporal system without adding entities. The collapsed toggle provides passive awareness (count), while the expanded list enables active exploration. Integration with the time slider means visitors can scrub through the day and see features light up. Click-to-fly reuses the existing flyToAttraction pattern for zero new camera logic.
- **How to encourage visitors to explore all 30 landmarks?** → Gamification via discovery progress tracker — localStorage Set, progress bar in Explore panel, checkmarks, toast notifications, completion celebration — With 30 landmarks, most visitors will only click a few. A progress tracker turns passive browsing into active exploration by showing how many remain undiscovered. The pattern follows the proven localStorage approach from Session 152 (baccarat bankroll). Hooking into the existing showLandmarkCard() means zero new user actions needed — any existing interaction counts. The Explore panel is the natural home for the progress bar since that's where landmarks are listed.
- **How to add wet-road neon reflection effect during rainy nights?** → CSS overlay at bottom 30% with gradient mask, base dark sheen layer + animated neon-colored gradient streaks, gated by _currentGlowIntensity > 0.1 AND rain detection, IIFE with 5s setInterval — CSS approach follows proven atmospheric effect pattern (rain splashes, harbor mist, sun rays). Using _currentGlowIntensity as night gate rather than raw hour check ensures consistency with time slider and existing neon system. Base sheen layer establishes the 'wet surface' so streaks read as reflections. IIFE self-containment matches rainbow/mist pattern. 12 casino-matched colors connect the effect to the city's actual landmarks.
